...
async def make_results(servers, results):
    pinged_servers = []
    for server in servers:
        ping_server = asyncio.create_task(ping(server, results))
    pinged_servers.append(ping_server)

    await asyncio.gather(*pinged_servers)


def ping_servers(servers):
    results = {"success": [], "failure": []}
    asyncio.run(make_results(servers, results))
    return results
